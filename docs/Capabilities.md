...
| Per-surface `@Reactive` state | Locally proven 2026-09-04 (ADR 0011): `ViewStateIdentityTests` (8 cases) pin inline persistence with an empty `transientStateIDs`, independent `WindowGroup` surfaces, per-surface writes from a hoisted instance, branch-flip eviction back to a zero baseline, out-of-band invalidation without `observe()`, the transient diagnostic naming its node, `.stateScope` surviving insertion, and host-less rendering staying local; `ReactiveStateLifetimeTests` and `SceneTests.sharedModelIndependentHosts` pass unchanged; 266 tests in 49 suites green; `check-boundaries.sh` requires the `~Sendable` store and slot, `check-concurrency-negative.sh` rejects a `Sendable` slot (3 fixtures), `check-embedded.sh` accepts the erasure, and `gama-demo --emit-mlir` is byte-identical to `origin/main`. Second backend: `gama-web-demo` uses the public `@Component`/`@Reactive` form; the Node smoke requires exactly `0` to `1`, and the browser marker requires `state=0->0->1`, attributing the mutation to Enter. Third backend: the macro-authored Android demo passes both ABI cross-builds and the API 36 emulator's exact `Tapped 0` to `Tapped 1` assertion | Hosted matrix on the merged commit |
| Strict memory safety + explicit imports | Locally proven 2026-09-04 (ADR 0012): every shipped library and macro target builds under `-strict-memory-safety` with the `StrictMemorySafety` group promoted to an error, and every Swift target under `InternalImportsByDefault`, with no diagnostics — Apple debug/test/release (266 tests), Embedded compile/link (unchanged 631,960 bytes), C ABI consumer, static Linux (`GamaCore` + `GamaTUI`, the Glibc branch), WASM SDK build plus Node and browser smokes (the wasm32 branch), iOS/tvOS/visionOS compile (the UIKit branch), boundaries, concurrency negatives, docs, and doc coverage | Hosted matrix on the merged commit. The Windows console branch of `GamaTUI` and the Android cross-compile were annotated by reading and rest on the Windows and Android jobs. Executables and `GamaTests` are deliberately outside the strict-memory scope (ADR 0012 records their measured counts); extending it is a separate decision |
