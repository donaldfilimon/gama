...
| Scene-first application core | Hosted proven: explicit primary selection, builder order, typed group payloads, unavailable window actions, lifecycle ordering, shared-model invalidation, and per-host cancellation are in the required matrix | Preserve the explicit-primary ABI tests; the approved per-surface view-state design remains separate, unimplemented work |
