...
hosted run `33931402885` at merge commit `bc2fe4d`, the current `origin/main`
tip, on 2026-09-04. Its WebAssembly job failed on the first attempt when
headless Chrome never published `DevToolsActivePort`; the same commit passed on
a re-run of that job, so the green result is the job's second attempt, not a
first-pass green. The preceding snapshot was run `33919361432` at merge commit
`77812d99`. Linux
...
| Embedded core | Locally compile/link proven: exact 2026-08-21 Swift 6.5-dev snapshot, bare-metal ARM ELF whole-module object and relocatably linked artifact. Measured 631,960 bytes on 2026-09-04 with the per-host `@Reactive` state store (+47,276 bytes, +8.1%, over the same-day `origin/main` baseline of 584,684 bytes — the identity store is the cost), and 636,792 bytes at unpushed commit `5dbdad8`. | Keep hosted Ubuntu artifact job green; no physical-board claim |
