...
| Plugin runtime + capability model (Tier 1) | Implemented and locally proven: 34 Swift Testing cases across runtime (deny-by-default, all-or-nothing, exact-match, ABI, service fail-closed, ownership isolation, deinit deactivation, per-host invalidation), slots (IR, actions, identity, reactive path), namespaced scene contributions with primary-role rejection and offscreen AppKit shell open/focus, deterministic commands, and hostile-path filesystem containment; demo slot wired through HostServices.standard | Hosted proof is the six-job acceptance matrix on the integrating PR. Tier 2 (dylib) and Tier 3 (out-of-process) are Proposed only; `.network` is reserved; no sandbox claim exists for in-process tiers; shell teardown of contributed windows on uninstall is deferred |
...
| Packaged wasm site | Locally proven (2026-08-27): `scripts/bundle-web.sh` assembles `index.html` + `gama.js` + `gama-web-demo.wasm` into `$GAMA_DIST_ROOT/web` with the pinned SDK and the headless-Chrome browser smoke passes against the assembled directory | Keep the hosted wasm job's bundle step and `wasm-site` upload green |
| macOS `.app` bundle (ad-hoc) | Locally proven (2026-08-27): `scripts/bundle-macos.sh` end to end on the pinned toolchain: manifest-driven Info.plist generation, `plutil -lint`, ad-hoc codesign with deep-strict verify, and the `--smoke` offscreen launch gate (non-empty DrawList, exit 0). Ad-hoc output runs on the building machine only; no Gatekeeper claim | Keep the hosted macOS job's bundle step and `macos-app` upload green. Developer ID + notarization are credential-gated: `scripts/release-macos.sh` is implemented and its fail-closed gating is locally proven, but no credentialed run has occurred; the notarized artifact is not claimed until one passes |
