...
| --- | --- | --- |
| wasm site directory works in a browser | `node scripts/browser-runtime-smoke.mjs "$DIST/web/gama-web-demo.wasm" "$DIST/web" "$WEB_TITLE"` run by `bundle-web.sh` against the assembled directory (headless Chrome: title branding, DOM, key, pointer, resize, rAF, accessibility, frames) | Locally proven by `bundle-web.sh`; the acceptance WebAssembly job independently rebuilds, smokes, and uploads the same directory, and the anchored hosted claim lives in the `WebAssembly/browser` row of [`Capabilities.md`](Capabilities.md), which `scripts/check-evidence-freshness.sh` checks for staleness. Do not restate that row's layer here, because nothing gates this file: a 2026-09-22 audit found this cell saying "hosted" while the row sat at `unverified` |
| GitHub Pages serves the interactive demo | `.github/workflows/pages.yml` rebuilds through `bundle-web.sh`, uploads only the verified `web/` directory, and deploys it to `https://donaldfilimon.github.io/gama/` from `main` | Hosted proven. The anchored claim, its evidence layer, and the paths it depends on live in the `Packaged wasm site` row of [`Capabilities.md`](Capabilities.md), which `scripts/check-evidence-freshness.sh` checks for staleness; do not restate the anchor or a measured artifact size here, because nothing gates this file. The byte figure this cell used to carry was removed from the ledger as stale on 2026-09-06: both `scripts/bundle-web.sh` and the wasm source it bundles changed after the run that produced it, and no re-measurement has been taken |
