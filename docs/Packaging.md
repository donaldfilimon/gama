...
| GitHub Pages serves the interactive demo | `.github/workflows/pages.yml` rebuilds through `bundle-web.sh`, uploads only the verified `web/` directory, and deploys it to `https://donaldfilimon.github.io/gama/` from `main` | Hosted proven 2026-09-04 at merge commit `77812d99`: Pages run `33919361438` built, browser-smoked, uploaded, and deployed; a separate live fetch returned `<title>Gama</title>` plus an HTTP 200 `application/wasm` payload of 9,297,539 bytes |
